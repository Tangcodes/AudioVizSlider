...
public class PlayerController implements Initializable {
    ...
    private final DecimalFormat oneDigit = new DecimalFormat("0.0");
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        timeSlider.valueProperty().addListener((ObservableValue<? extends Number> ov, Number old_val, Number new_val) -> {
           if(timeSlider.isValueChanging()){
                currentTime = Double.parseDouble(oneDigit.format(timeSlider.valueProperty().getValue()));
                currentText.setText( currentTime + " ms");
    ...
    private void handleReady() {
        ...
        durationFormat = durationFormat.replaceAll("[^\\d.]", "");
        lengthText.setText(oneDigit.format(Double.parseDouble(durationFormat)) + " ms");
